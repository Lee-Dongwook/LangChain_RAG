...
def build_loaders(root: str, batch_size: int = 64, val_ratio: float = 0.2, seed: int = 42):
    generator = torch.Generator().manual_seed(seed)

    full_train_dataset = CifakeDataset(root=root, transform=get_train_transform())
    full_val_dataset = CifakeDataset(root=root, transform=get_eval_transform())
